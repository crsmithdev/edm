...
- [x] Add `kick` event detection using `librosa.onset.onset_detect()` for percussion onsets
- [x] Filter kick events to only include strong onsets (threshold by onset strength)
- [x] Add kick events to output in `analyze_structure()`
...
- [x] Add test for kick event detection
...
- [x] Update `src/edm/evaluation/evaluators/structure.py` to handle event format
- [x] Add event evaluation metrics (precision/recall for drops, kicks)
- [x] Update `load_structure_reference()` to parse both span and event formats
- [x] Test evaluation on annotated data

## Phase 6: Documentation and Migration

- [x] Migrate existing annotations in `data/` to 1-indexed format
- [x] Update `STRUCTURE_ANNOTATIONS_GUIDE.md` with new format and conventions
- [x] Update CLI help text and examples
- [x] Add CHANGELOG entry documenting breaking changes
- [x] Update README with example output showing new format
